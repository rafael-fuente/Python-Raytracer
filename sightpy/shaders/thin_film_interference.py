...
class ThinFilmInterference(Shader):
    def __init__(self, thickness, max_ray_depth, noise = 0.):
    ...
    def get_color(self, scene, ray, hit):
        ...
        if ray.depth < self.max_ray_depth:
            ...
            color += (scene.ambient_color + raytrace(Ray(nudged, reflected_ray_dir, ray.depth + 1, ray.n, ray.reflections + 1, ray.transmissions), scene))*F
            ...
            T = 1. - F
            transmitted_color = raytrace( Ray(nudged, transmitted_ray_dir, ray.depth + 1, ray.n, ray.reflections, ray.transmissions + 1), scene  )*T
